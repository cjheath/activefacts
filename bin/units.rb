#! /usr/bin/ruby
#
#       ActiveFacts: Units data file loader (test program).
#
...
class Units
  ...
  class Unit
    attr_reader :name, :coefficient, :is_precise, :baseunits

    def initialize(name, coefficient, is_precise, baseunits)
      @name = name
    ...
    def to_s
      "#{coefficient.to_s} #{baseunits.map{|b| b.to_s} * " "}" +
      " converts to #{name}" +
  ...
  def initialize(file = '/usr/share/misc/units.lib')
    ...
    File.open(file) { |f|
      f.each_line { |line|
        ...
        @units[unit] = Unit.new(unit, coefficient, is_precise, baseunits)
...
#       puts "#{unit} #{is_precise ? "=" : "~~"} #{coefficient != 1 ? coefficient.to_s : ""} #{baseunits.map{|b| b.to_s} * " "}"
  ...
  def dump
    units = @units.clone
    while units.size > 0
      units.keys.sort.each do |unit_name|
        unit = units[unit_name]
        if unit != []    # Fundamental unit
          next if unit.baseunits.detect{|b| units[b.unit]}   # Precursor not dumped
          puts unit
        end
        units.delete(unit_name)
      end
    end
  end
