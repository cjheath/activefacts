    ...
    class Compiler < ActiveFacts::CQL::Parser
      class Definition
        ...
        def build_variables(clauses_list)
          debug :join, "Building variables" do
            join = @constellation.Join(:new)
            all_bindings_in_clauses(clauses_list).
              each do |binding|
                debug :join, "Creating variable #{join.all_variable.size} for #{binding.inspect}"
                binding.variable = @constellation.Variable(join, join.all_variable.size, :object_type => binding.player)
        ...
        def build_steps clause, roles_by_binding = {}, objectification_node = nil
          plays = []
          incidental_roles = []
          debug :join, "Creating Role Sequence for #{clause.inspect} with #{clause.refs.size} role refs" do
            ...
            clause.refs.each do |ref|
              ...
              if (clause.fact_type.entity_type)
                ...
                debug :join, "Creating Variable #{ref.inspect} (counts #{refs_count}/#{objectification_ref_count}) and objectification Step for #{ref.inspect}" do
                  ...
                  unless objectification_node
                    ...
                    join = binding.variable.join
                    debug :join, "Creating JN#{join.all_variable.size} for #{clause.fact_type.entity_type.name} in objectification"
                    objectification_node = @constellation.Variable(join, join.all_variable.size, :object_type => clause.fact_type.entity_type)
                  end
                ...
                debug :join, "Creating Reference for #{ref.inspect}" do
              ...
              if ref.nested_clauses
                ...
                ref.nested_clauses.each do |r|
                  debug :join, "Building objectification join for #{ref.nested_clauses.inspect}" do
          ...
          if plays.size > 0
            ...
            raise "REVISIT: Internal error constructing join for #{clause.inspect}" if plays.size != 2
