...
describe "Business Context Notes" do
  ...
  Notes = [
    ...
    [ 'Foo is identified by Bar [independent] where Foo has one Bar (so that we have an id);',
      [["Person", [:value_type, "Person", [], nil, [], []]], ["Foo", [:entity_type, [], {:roles=>[["Bar"]]}, ["independent"], [[:fact_clause, [], [{:word=>"Foo"}, {:word=>"has"}, {:word=>"Bar", :quantifier=>[1, 1]}], [nil, "so_that", " we have an id", []]]]]]]
    ],
    [ 'Foo has one Bar (so that we have an id), Bar is of one Foo (because we need that);',
      [["Person", [:value_type, "Person", [], nil, [], []]], [nil, [:fact_type, [[:fact_clause, [], [{:word=>"Foo"}, {:word=>"has"}, {:word=>"Bar", :quantifier=>[1, 1]}], [nil, "so_that", " we have an id", []]], [:fact_clause, [], [{:word=>"Bar"}, {:word=>"is"}, {:word=>"of"}, {:word=>"Foo", :quantifier=>[1, 1]}], [nil, "because", "we need that", []]]], []]]]
    ],
